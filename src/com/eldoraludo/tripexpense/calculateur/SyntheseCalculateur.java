...
public class SyntheseCalculateur implements Calculateur<List<SyntheseDTO>> {
	...
	public List<SyntheseDTO> run() {
		...
		Map<Participant, Map<Participant, Double>> detteFinal = nettoyageDesRelationsSymetriques(dette);

		Map<Participant, Map<Participant, Double>> detteSimplifie = extractionDetteSimplifierParParticipant(detteFinal);
	...
	public Map<Participant, Map<Participant, Double>> extractionDetteSimplifierParParticipant(
		...
		for (int i = 0; i < 4; i++) {
			List<Participant> dettes;
			if (i % 2 == 0) {
				dettes = Lists.reverse(new LinkedList<Participant>(
						detteSimplifier.keySet()));
			} else {
				dettes = new LinkedList<Participant>(detteSimplifier.keySet());
			}
			for (Participant participant : dettes) {
				...
				for (Participant depenseur : depenseurs) {
					...
					SetView<Participant> intersection = Sets
